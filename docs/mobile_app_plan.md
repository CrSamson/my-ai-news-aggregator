...
#### F1 — Expo Web production build ✅ shipped

- `npm run build:web` (defined in [mobile/package.json](mobile/package.json)) wraps `expo export --platform web --output-dir dist`.
- Local preview: `npm run serve:web` → http://localhost:4173.
- Output lives at `mobile/dist/` (gitignored).

#### F2 — PWA manifest + service worker ✅ shipped

- Web manifest is generated by Expo from [mobile/app.json](mobile/app.json) `web` section (name, shortName, themeColor, backgroundColor, display: standalone, lang).
- Service worker at [mobile/public/sw.js](mobile/public/sw.js): hand-rolled (~70 lines), two strategies:
  - **App shell** (HTML, JS, CSS, fonts, images): cache-first, evicted on `CACHE_VERSION` bump.
  - **`/api/v1/*` + Modal hosts**: stale-while-revalidate — instant render from cache, silent background refresh. Right call for a once-a-day product.
- Registered via [mobile/lib/registerSW.ts](mobile/lib/registerSW.ts), imported for its side-effect from `app/_layout.tsx`. Web-only + dev-gated so `expo start --web` doesn't 404 on `/sw.js`.

#### F3 — Cloudflare Pages deployment

Build configuration (set once in the CF Pages dashboard):
- **Framework preset**: None.
- **Root directory**: `mobile`.
- **Build command**: `npm install && npm run build:web`.
- **Output directory**: `dist` (resolves to `mobile/dist` from the root directory).
- **Node version**: 20 (set via env var `NODE_VERSION=20` if CF defaults stick at 18 for legacy projects).
- **Environment variables**: `EXPO_PUBLIC_API_URL=https://crsamson--brevio-api-fastapi-app.modal.run`.

Edge config files in [mobile/public/](mobile/public/) (Expo copies `public/` verbatim into `dist/`):
- [_redirects](mobile/public/_redirects) — SPA fallback `/* /index.html 200` so deep links don't 404.
- [_headers](mobile/public/_headers) — `no-cache` on `/sw.js` + `/manifest.json` (the classic stale-SW footgun); `must-revalidate` on `/` and `/index.html`; `immutable` 1-year cache on hashed `/_expo/static/*` and `/assets/*`.

First deploy → CF assigns `https://<project>.pages.dev`. Optional custom domain via CF DNS (free).
...
- Turn on airplane mode → open the app → last cached feed should appear (SWR returns the stale-cached response).
